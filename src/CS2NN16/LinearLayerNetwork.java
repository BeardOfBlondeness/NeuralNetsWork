...
public class LinearLayerNetwork {
	...
	protected void findDeltas(ArrayList<Double> errors) {
		...
		for(int n = 0; n < numNeurons; n++) deltas.set(n, errors.get(n));
	}	
	...
	private int weightIndex (int wNeuron, int wWeight) {
		return  wWeight + wNeuron * (numInputs+1);
	}
	...
	private void changeOneWeight(int wNeuron, int wWeight, double theIn, double learnRate, double momentum) {
		...
		int weightIndex = weightIndex(wNeuron, wWeight);
		changeInWeights.set(weightIndex, theIn * deltas.get(wNeuron) * learnRate + momentum * changeInWeights.get(weightIndex));
		weights.set(weightIndex, weights.get(weightIndex) + changeInWeights.get(weightIndex));
	}
	...
	protected void changeAllWeights(ArrayList<Double> ins, double learnRate, double momentum) {
		...
		for(int n = 0; n < numNeurons; n++) {
			for(int ct = 0; ct < numInputs+1; ct++) {
				if(ct == 0) theIn = 1;
				else theIn = ins.get(ct-1);
				changeOneWeight(n, ct, theIn, learnRate, momentum);
			}
		}
	}
	...
	public String getWeights() {
		...
		for(int i = 0; i < numWeights; i++) s += String.format("%.5f", weights.get(i)) + " ";
