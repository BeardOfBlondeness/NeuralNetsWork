...
public class MultiLayerNetwork extends SigmoidLayerNetwork {
	...
	protected void calcOutputs(ArrayList<Double> nInputs) {
		super.calcOutputs(nInputs); //Calculate outputs for this layer
		nextLayer.calcOutputs(super.outputs); //Calculate outputs for next layer
	}
	...
	protected void outputsToDataSet (int ct, DataSet d) {
		d.storeOutputs(ct, nextLayer.outputs); //Stores the outputs of the next layer in the data set
	}
	...
	protected void findDeltas(ArrayList<Double> errors) {
		nextLayer.findDeltas(errors); //find deltas for next layer using set errors
		super.findDeltas(nextLayer.weightedDeltas()); //find deltas for this layer using weighted deltas of next 
	}
	...
	protected void changeAllWeights(ArrayList<Double> ins, double learnRate, double momentum) {
		super.changeAllWeights(ins, learnRate, momentum); //change weights for this layer
		nextLayer.changeAllWeights(super.outputs, learnRate, momentum); //change weights for next layer using outputs from this layer as inputs
	}	
	...
	public int numWeights() {
		return numWeights + nextLayer.numWeights(); //Gets the current number of weights and adds the next layers number of weights
	}
	...
	public String getWeights() {
		return super.getWeights() + nextLayer.getWeights();	//Gets the weights for the current layer and concatinates with the next layers weights
	}
